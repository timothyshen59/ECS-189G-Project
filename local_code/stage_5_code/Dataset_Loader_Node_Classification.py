...
class Dataset_Loader(dataset):
    ...
    def stratified_split(self, labels, train_per_class, test_per_class, seed=42):
        ...
        for c in classes:
            ...
            idx_train.extend(idx[:train_per_class])
            idx_test.extend(idx[train_per_class:train_per_class + test_per_class])

        used_idx = set(idx_train + idx_test)
        all_idx = set(range(len(labels_np)))
        idx_val = list(all_idx - used_idx)
    ...
    def load(self):
        ...
        if self.dataset_name == 'cora':
            idx_train, idx_val, idx_test = self.stratified_split(labels, train_per_class=20, test_per_class=150)

        elif self.dataset_name == 'citeseer':
            idx_train, idx_val, idx_test = self.stratified_split(labels, train_per_class=20, test_per_class=200)
